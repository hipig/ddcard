...
@section('content')
  <div class="flex flex-col rounded shadow-sm bg-white overflow-hidden">
    <div class="py-4 px-5 lg:px-6 flex-grow w-full bg-gray-50">
      <span class="text-gray-900">关于我们添加</span>
    </div>
    <div class="p-5 lg:p-6 flex-grow w-full">
      <form action="{{ route('admin.abouts.store') }}" method="post" class="space-y-6">
        <div hidden>
          @csrf
        </div>
        ...
        <div class="space-y-1 md:space-y-0 md:flex md:items-center">
          ...
          <input type="text" id="key" name="key" value="{{ old('key') }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full md:w-3/5 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="请输入标识" />
        </div>
        <div class="space-y-1 md:space-y-0 md:flex md:items-baseline" x-data="{content: `{{ old('content', '') }}`}" x-init="quill = new Quill($refs.quillEditor, {
            modules: {
              toolbar: {
                container: [
